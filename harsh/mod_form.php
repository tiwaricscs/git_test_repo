...
class mod_harsh_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        $mform->addElement('url', 'externalurl', get_string('externalurl', 'harsh'), array('size' => '60'), array('usefilepicker' => true));
        $mform->setType('externalurl', PARAM_RAW_TRIMMED);
        $mform->addRule('externalurl', null, 'required', null, 'client');
        $mform->addElement('checkbox', 'userid', get_string('userid', 'harsh'), 'user id', null, array(0, 1));
        $mform->setType('userid', PARAM_INT);
        $mform->addElement('checkbox', 'courseid', get_string('courseid', 'harsh'), 'course id', null, array(0, 1));
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'harsh'), $options);
            $mform->setDefault('display', $config->display);
            $mform->addHelpButton('display', 'displayselect', 'harsh');
        }
        if (array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('text', 'iframewidth', get_string('iframewidth', 'harsh'), array('size' => 3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'iframeheight', get_string('iframeheight', 'harsh'), array('size' => 3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
            ...
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'harsh'));

            $mform->setDefault('printintro', $config->printintro);
        }
        $mform->addElement('header', 'parameterssection', get_string('parametersheader', 'harsh'));
        $mform->addElement('static', 'parametersinfo', '', get_string('parametersheader_help', 'harsh'));
        ...
        $options = harsh_get_variable_options($config);
        ...
        for ($i = 0; $i < $parcount; $i++) {
            ...
            $mform->addGroup($group, $pargroup, get_string('parameterinfo', 'harsh'), ' ', false);
    ...
    function validation($data, $files) {
        ...
        if (!empty($data['externalurl'])) {
            ...
            if (preg_match('|^/|', $url)) {
                ...
                if (!harsh_appears_valid_url($url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'harsh');
                }
            } else if (preg_match('|^[a-z]+:|i', $url)) {
                
            } else {
                if (!harsh_appears_valid_url('http://' . $url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'harsh');
                }
            }
        }
