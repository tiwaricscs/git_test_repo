 ...
 */
class moodle1_mod_harsh_handler extends moodle1_resource_successor_handler {
...
//        } 
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_harsh', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // write url.xml
        $this->open_xml_writer("activities/harsh_{$moduleid}/harsh.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'harsh', 'contextid' => $contextid));
        $this->write_xml('harsh', $url, array('/harsh/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/harsh_{$moduleid}/inforef.xml");
