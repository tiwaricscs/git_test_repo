...
function myurllactivity_appears_valid_url($url) {
...
function myurllactivity_fix_submitted_url($url) {
    $url = trim($url);
...
function myurllactivity_get_full_url($url, $cm, $course, $config = null) {
    ...
    if (preg_match('/^(\/|https?:|ftp:)/i', $fullurl) or preg_match('|^/|', $fullurl)) {
        $allowed = "[" . $letters . $latin . $digits . $symbols . $arabic . $math . $othernumbers . $geometric .
                $emojis . "]" . preg_quote(';/?:@=&$_.+!*(),-#%', '/');
        $fullurl = preg_replace_callback("/[^$allowed]/u", 'myurllactivity_filter_callback', $fullurl);
    } else {
    ...
    if (!empty($parameters)) {
        ...
        $paramvalues = myurllactivity_get_variable_values($url, $cm, $course, $config);
...
function myurllactivity_filter_callback($matches) {
    return rawurlencode($matches[0]);
}
function myurllactivity_print_header($url, $cm, $course) {
    global $PAGE, $OUTPUT;
...
function myurllactivity_print_heading($url, $cm, $course, $notused = false) {
    global $OUTPUT;
    echo $OUTPUT->heading(format_string($url->name), 2);
}
function myurllactivity_print_intro($url, $cm, $course, $ignoresettings = false) {
    global $OUTPUT;
    $options = empty($url->displayoptions) ? array() : unserialize($url->displayoptions);
    if ($ignoresettings or!empty($options['printintro'])) {
        if (trim(strip_tags($url->intro))) {
            echo $OUTPUT->box_start('mod_introbox', 'urlintro');
            echo format_module_intro('myurllactivity', $url, $cm->id);
            echo $OUTPUT->box_end();
        }
    }
}
function myurllactivity_display_frame($url, $cm, $course) {
    // global $PAGE, $OUTPUT, $CFG;
    // $a = unserialize($url->displayoptions);
    // $frame = optional_param('frameset', 'main', PARAM_ALPHA);
    // if ($frame === 'top') {
    //     $PAGE->set_pagelayout('frametop');
    //     myurllactivity_print_header($url, $cm, $course);
    //     myurllactivity_print_heading($url, $cm, $course);
    //     myurllactivity_print_intro($url, $cm, $course);
    //     echo $OUTPUT->footer();
    //     die;
    // } else {
    //     $config = get_config('mod_myurllactivity');
    //     $context = context_module::instance($cm->id);
    //     $exteurl = myurllactivity_get_full_url($url, $cm, $course, $config);
    //     $navurl = "$CFG->wwwroot/mod/myurllactivity/view.php?id=$cm->id&amp;frameset=top";
    ...
    //         $store = get_config('mod_myurllactivity');
    ...
    //     }
    //     $modulename = s(get_string('modulename', 'myurllactivity'));
    //     $contentframetitle = s(format_string($url->name));
    //     $dir = get_string('thisdirection', 'langconfig');
    //     myurllactivity_print_header($url, $cm, $course);
    //     myurllactivity_print_heading($url, $cm, $course);
    //     myurllactivity_print_intro($url, $cm, $course);
    //     $img = $CFG->wwwroot . '/mod/myurllactivity/pix/waiting.gif';
    ...
    if ($frame === 'top') {
        ...
        $curl = new curl();
        $config = get_config('myurllactivity');
        $exteurl = myurllactivity_get_full_url($url, $cm, $course, $config);
        ...
        // echo "<style>.loader_main_overlay{ z-index: -1;  position: absolute; }</style>";

        echo html_writer::start_tag('div');
        echo html_writer::start_tag('img', array('src' => $CFG->wwwroot.'/mod/myurllactivity/pix/loader.gif',
            'class' => 'loader_main_overlay', 'url' => $exteurl));
        $result = $curl->get($exteurl);
        echo "<iframe id='load' width=\"$width\" height=\"$height\">$result</iframe>";
      //  echo "$result";
        echo html_writer::end_tag('div');
        $PAGE->requires->js_call_amd('mod_myurllactivity/main', 'loader');
        echo $OUTPUT->footer();
        die;
    }
    }

function myurllactivity_print_workaround($url, $cm, $course) {
    global $OUTPUT;
    myurllactivity_print_header($url, $cm, $course);
    myurllactivity_print_heading($url, $cm, $course, true);
    myurllactivity_print_intro($url, $cm, $course, true);
    $fullurl = myurllactivity_get_full_url($url, $cm, $course);
    $display = myurllactivity_get_final_display_type($url);
    ...
    print_string('clicktoopen', 'myurllactivity', "<a href=\"$fullurl\" $extra>$fullurl</a>");
    echo '</div>';
    echo $OUTPUT->footer();
    die;
}
function myurllactivity_display_embed($url, $cm, $course) {
    global $CFG, $PAGE, $OUTPUT;
    $mimetype = resourcelib_guess_url_mimetype($url->externalurl);
    $fullurl = myurllactivity_get_full_url($url, $cm, $course);
    $title = $url->name;
    $link = html_writer::tag('a', $fullurl, array('href' => str_replace('&amp;', '&', $fullurl)));
    $clicktoopen = get_string('clicktoopen', 'myurllactivity', $link);
...
function myurllactivity_get_final_display_type($url) {
    global $CFG;
...
function myurllactivity_get_variable_options($config) {
    global $CFG;
    $options = array();
    $options[''] = array('' => get_string('chooseavariable', 'myurllactivity'));
    ...
    $options[get_string('modulename', 'myurllactivity')] = array(
        'urlinstance' => 'id',
        'urlcmid' => 'cmid',
    ...
    $options[get_string('miscellaneous')] = array(
        ...
        'serverurl' => get_string('serverurl', 'myurllactivity'),
...
function myurllactivity_get_variable_values($url, $cm, $course, $config) {
    global $USER, $CFG;
    $site = get_site();
    $coursecontext = context_course::instance($course->id);
    $values = array(
    ...
    if (!empty($config->secretphrase)) {
        $values['encryptedcode'] = myurllactivity_get_encrypted_parameter($url, $config);
    }
    if ($config->rolesinparams) {
        $coursecontext = context_course::instance($course->id);
...
function myurllactivity_get_encrypted_parameter($url, $config) {
    global $CFG;
...
function myurllactivity_guess_icon($fullurl, $size = null) {
    global $CFG;
    require_once("$CFG->libdir/filelib.php");
