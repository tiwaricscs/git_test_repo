 ...
 */
class moodle1_mod_myurlactivity_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource(array $data, array $raw = null) {
        ...
        if ($data['options'] == 'frame') {
            $url['display'] = ACTIVITYLIB_DISPLAY_FRAME;

        } else if ($data['options'] == 'objectframe') {
            $url['display'] = ACTIVITYLIB_DISPLAY_EMBED;

        } else {
            $url['display'] = ACTIVITYLIB_DISPLAY_AUTO;
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_myurlactivity', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // Write url.xml.
        $this->open_xml_writer("activities/myurlactivity_{$moduleid}/myurlactivity.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'myurlactivity', 'contextid' => $contextid));
        $this->write_xml('myurlactivity', $url, array('/myurlactivity/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // Write inforef.xml.
        $this->open_xml_writer("activities/myurlactivity_{$moduleid}/inforef.xml");
